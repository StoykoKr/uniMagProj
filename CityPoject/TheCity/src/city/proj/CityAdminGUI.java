...
public class CityAdminGUI extends JFrame implements PlaceClickListener {
	...
	private JPanel createPersonDetailsPanel() {
		...
		saveTo.addActionListener((ActionEvent e) -> {
			if (currentlyViewedPerson != null) {
				String newName = nameTextField.getText().trim();
				if (!newName.isEmpty()) {
					currentlyViewedPerson.setName(newName);
					agent.db.insertPerson(currentlyViewedPerson);
	...
	private JPanel mapSetupMode() {
		...
		list.addListSelectionListener(e -> {
			if (!e.getValueIsAdjusting()) {
				...
				if (selected != null) {
					boolean alreadyIn = agent.people.stream().anyMatch(p -> p.getId() == selected.getId());
					if (!alreadyIn) {
						agent.people.add(selected);
						agent.ensureImigrantsAreWelcome(selected);
						System.out.println("Added: " + selected.getName());
					} else {
						System.out.println("Already added: " + selected.getName());
					}
				}
			}
		});
