...
public class CityOntology {
	...
	private void addDataProperty(String propName, OWLNamedIndividual subject, Object value) {
		...
		if (value instanceof String) {
			...
			literal = dataFactory.getOWLLiteral(Integer.toString((Integer) value), OWL2Datatype.XSD_INT); 
	...
	public void updateLocation(String personName, String placeName) {
	    ...
	    Set<OWLObjectPropertyAssertionAxiom> existingAxioms = cityOntology.getObjectPropertyAssertionAxioms(person);
	    for (OWLObjectPropertyAssertionAxiom ax : existingAxioms) {
	        if (ax.getProperty().equals(isLocatedAt)) {
	        	ontoManager.removeAxiom(cityOntology, ax);
	        }
	    }

	    OWLObjectPropertyAssertionAxiom newAxiom =
	        dataFactory.getOWLObjectPropertyAssertionAxiom(isLocatedAt, person, newPlace);
	    ontoManager.addAxiom(cityOntology, newAxiom);
	}
